...
class HomeFragment : Fragment() {
    ...
    private fun testConnection() {
        val bottomSheet = TestConnectionBottomSheet(viewModel)
        bottomSheet.show(parentFragmentManager, TestConnectionBottomSheet.TAG)
    }
    ...
    private fun refreshIpInfo(retryCount: Int = 0) {
        android.util.Log.d("HomeFragment", "refreshIpInfo called, retryCount: $retryCount")
        viewLifecycleOwner.lifecycleScope.launch {
            try {
